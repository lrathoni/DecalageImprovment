...
public class GrabSeagull : MonoBehaviour
{
    ...
    {
        Vector3 destination = (seagull.TransformPoint(Vector3.back + Vector3.down)) * dragDistance;
        Vector3 direction = (destination - transform.root.position);
        direction.Normalize();
        transform.root.position += direction * speed;
    }
}
